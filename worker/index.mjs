...
function requireProtocol(request) {
  return request.headers.get('X-VS-Protocol') === PROTOCOL;
}
...
function mediaKey(pathname) {
  ...
  let value;
  try { value = decodeURIComponent(encoded); } catch { throw new Error('invalid-media-pointer'); }
...
function mediaSuccess(key, status = 201, replay = false) {
  return json(
    { ok: true, pointer: key, url: `/api/media/${encodeURIComponent(key)}` },
    status,
