...
export class StudioState {
  ...
  eventSocket(request) {
    ...
    this.ctx.acceptWebSocket(server, ['studio-events']);
    const record = this.record();
    server.send(JSON.stringify({ type: 'hello', mode: record?.mode || 'missing', revision: record?.revision ?? null }));
    return new Response(null, { status: 101, webSocket: client });
  }
  broadcast(record) {
    if (typeof this.ctx.getWebSockets !== 'function') return;
    const message = JSON.stringify({ type: 'state-changed', mode: record?.mode || 'missing', revision: record?.revision ?? null });
    for (const socket of this.ctx.getWebSockets('studio-events')) { try { socket.send(message); } catch {} }
  }
  ...
  webSocketError(socket) { try { socket.close(1011, 'socket-error'); } catch {} }
  async getState() {
    const record = this.record();
  ...
  async importState(request) {
    ...
    const body = await request.json(); const state = cleanIncomingState(body.state);
    const record = { mode: 'staged', revision: 0, state };
    this.setKV('record', record); this.setKV('readback', null); this.broadcast(record);
    return json({ ok: true, mode: record.mode, revision: 0, hash: await sha256(state) }, 201);
  }
  async exportState(request) {
    const record = this.record(); if (!record) return json({ error: 'missing-state' }, 404);
  ...
  async activate(request) {
    ...
    const principal = request.headers.get('X-VS-Principal') || '';
    const readback = this.getKV('readback'); const hash = await sha256(record.state);
    ...
    this.setKV('record', next); this.broadcast(next);
  ...
  async commit(request) {
    ...
    if (body.baseRevision !== record.revision) return json({ error: 'revision-conflict', currentRevision: record.revision }, 409);
