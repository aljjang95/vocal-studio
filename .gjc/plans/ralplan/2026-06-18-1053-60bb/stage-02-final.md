# Pending Approval Plan — Vocal Studio A-to-Z Upgrade

Status: pending approval  
Run ID: 2026-06-18-1053-60bb  
Created: 2026-06-18T11:33:28.123270+00:00

## Consensus receipts
- Planner stage 1: `.gjc/plans/ralplan/2026-06-18-1053-60bb/stage-01-planner.md`
- Architect pass 1: BLOCK / REQUEST CHANGES — `.gjc/plans/ralplan/2026-06-18-1053-60bb/stage-03-architect.md`
- Critic pass 1: ITERATE — `.gjc/plans/ralplan/2026-06-18-1053-60bb/stage-01-critic.md`
- Revision pass 2: `.gjc/plans/ralplan/2026-06-18-1053-60bb/stage-02-revision.md`
- Architect pass 2: CLEAR / APPROVE — `.gjc/plans/ralplan/2026-06-18-1053-60bb/stage-04-architect.md`
- Critic pass 2: APPROVE — `.gjc/plans/ralplan/2026-06-18-1053-60bb/stage-02-critic.md`

## ADR

### Decision
Adopt Option A: security-first staged modernization of the current `index.html` runtime. The first executable slice is indivisible: remove private PII from all public static assets, replace anonymous broad access with authorized access/rules, purge old service-worker cache exposure, and disable render/navigation-triggered automatic attendance writes. Subsequent phases introduce attendance proposals, unified cycle/payment logic, split Firestore collections with migration freeze/live-hash checks, media manifest/recovery, schedule engine v2, first-class group lessons, and source-of-truth cleanup.

### Drivers
1. Private operational data is present in public static assets and old service-worker caches, not only in Firestore.
2. Rendering Today/schedule can mutate attendance, which then affects billing/cycle state.
3. The single `/studio/data` document, media placeholders, stale modules, and ambiguous schedule/group model make large rewrites risky without staged gates.

### Alternatives considered
- Big-bang modular rewrite and schema replacement: cleaner end state, rejected for initial execution because it delays P0 security and billing fixes and has high regression/backout risk.
- Emergency lockdown only: useful as a first slice, rejected as the full plan because it leaves data overwrite, media, cycle, schedule, and group risks.

### Why chosen
This path reduces the highest operational risk first while preserving the existing operator workflow and enabling verified, reversible migration. It explicitly prevents new attendance corruption in the same first production slice as security hardening.

### Consequences
- Early work stays in/around `index.html` until modules are wired into runtime.
- Public asset scanning, auth/rules tests, no-write render tests, and service-worker cache purge become mandatory release gates.
- Migration cannot proceed without freeze/maintenance window, live hash/revision checkpoint, media manifest, and rollback pointer.

### Follow-ups
- Use `ultragoal` for approved multi-phase execution unless tmux-based parallel workers are explicitly required.
- Run Architect review after Phase 1 and before split Firestore cutover.
- Run Critic review before migration cutover to challenge freeze/hash/media quarantine/backout readiness.
